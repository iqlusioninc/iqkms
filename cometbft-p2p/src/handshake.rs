...
/// Transcript hashes computed using Merlin.
type TranscriptHash = [u8; 32];
...
impl InitialState {
    ...
    pub fn got_key(
        ...
        let mut transcript_hash = TranscriptHash::default();
        transcript.challenge_bytes(b"SECRET_CONNECTION_MAC", &mut transcript_hash);

        Ok((AwaitingResponse { transcript_hash }, cipher_state))
    }
}
...
impl AwaitingResponse {
    ...
    {
        signer.sign(&self.transcript_hash)
    }
    ...
    pub fn got_signature(&self, auth_sig_msg: proto::p2p::AuthSigMessage) -> Result<PublicKey> {
        ...
        peer_pubkey.verify(&self.transcript_hash, &auth_sig_msg.sig)?;
